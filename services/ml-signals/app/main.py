...
def _features_from_prices(p):
    ...
    ret = np.diff(p) / p[:-1]  # daily returns

    # Compute simple moving averages
    sma5 = np.convolve(p, np.ones(5)/5, mode="valid")
    sma10 = np.convolve(p, np.ones(10)/10, mode="valid")

    # Align lengths to the shortest array to avoid broadcasting errors
    L = min(len(ret), len(sma5), len(sma10), len(p))
    sma5 = sma5[-L:]
    sma10 = sma10[-L:]
    p = p[-L:]
    ret = ret[-L:]
    ...
    X = np.column_stack([
        ...
        sma5 / p,
        sma10 / p,
        np.std(ret) * np.ones(L),
    ])

    # Generate synthetic labels (0 = sell, 1 = hold, 2 = buy)
    y = np.where((sma5 > sma10) & (ret > 0), 2, 1)
    y = np.where((sma5 < sma10) & (ret < 0), 0, y)
    return X, y
...
def _train_synth():
    ...
    Xs, ys = [], []
    for _ in range(20):
        base = 100 + rng.normal(0, 1, 80).cumsum()  # synthetic prices
